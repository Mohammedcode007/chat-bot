...
*/
const AUDIO_DELETE_EXTRA_MS = 2 * 60 * 1000; // دقيقتين بعد مدة الأغنية

/*
  احتياطي لو فشل ffprobe في معرفة مدة الأغنية
*/
const AUDIO_FALLBACK_TTL_MS = 10 * 60 * 1000; // 10 دقائق احتياطي
...
const fallbackDeleteAfterMs = AUDIO_FALLBACK_TTL_MS;

scheduleDeleteFile(absolutePath, fallbackDeleteAfterMs);

getAudioDurationMs(absolutePath)
  .then((audioDurationMs) => {
    ...
    /*
      ملاحظة:
      تم ترك الحذف الاحتياطي بالأعلى كما هو.
      هذا يحافظ على التنظيف حتى لو ffprobe تأخر أو فشل.
    */
  })
  .catch((error) => {
    console.error(
      "❌ async ffprobe duration error:",
      error && error.message ? error.message : error
    );
  });
...
return {
  ...
  publicUrl,
  durationMs: 0,
  expiresInMs: fallbackDeleteAfterMs,
};
}
...
/*
  هذه الدالة تنظف الملفات القديمة جدًا فقط.
  لأن الحذف الأساسي يتم بعد مدة الأغنية + دقيقتين.
  جعلناها 24 ساعة حتى لا تحذف أغنية شغالة بعد إعادة تشغيل البوت.
*/
function cleanupExpiredAudioFiles() {
  ensureDirExists(AUDIO_TEMP_DIR);

  const CLEANUP_OLD_FILES_MS = 24 * 60 * 60 * 1000; // 24 ساعة
